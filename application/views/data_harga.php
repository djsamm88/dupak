...
<table id="tbl_newsnya" class="table  table-striped table-bordered"  cellspacing="0" width="100%">
      ...
      <tbody>
        <?php         
        $no = 0;
        foreach($this->m_semua->m_kecamatan() as $x)
        {
          
          $no++;

            $har = "<table class='table'>
                    
                      ";
                     foreach ($this->m_semua->m_sub_kategori() as $sub) {
                        $har.="<tr>

                                  <td>
                                    $sub->sub_kategori 
                                    <input id='id_kecamatan' type='hidden' value='$x->kode'>
                                    <input id='id_sub_kategori' type='hidden' value='$sub->id'>
                                  </td>
                                  <td><input class='form-control nomor' id='harga'></td>
                                  <td id='t4_harga'></td>
                                  <td>
                                    <input class='btn btn-primary btn-xs' value='Update' type='button' onclick='gas_harga($(this));return false;'>
                                  </td>
                               </tr>";
                      }
