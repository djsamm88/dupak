...
function upload_file($name_field){
    ...
    if($ext=="jpg" || $ext=="jpeg" || $ext=="png" || $ext=="pdf" || $ext=="doc" || $ext=="docx" || $ext=="JPG" || $ext=="JPEG" || $ext=="PNG" || $ext=="PDF" || $ext=="DOC" || $ext=="DOCX" || $ext=="xls" || $ext=="xlsx"){
       // $fName = time();
        $fName = str_replace(array(" ","."), "_", $path).'_'.$name_field."_".time();
        $fileName = $fName.'.'.$ext;
